...
namespace FormSerialisation
{
    public static class FormSerialisor
    {
        /*
          ...
          FormSerialisor created by M. J. Spangler (Shardvexz), inspired by a question on stackoverflow.
        */
            ...
            foreach (Control childCtrl in c.Controls)
            {
                if (!(childCtrl is Label))
                {
                    ...
                    else if (childCtrl is ListBox)
                    {
                        ...
                        //Serialise all of the items in the listbox
                        foreach (string item in lst.Items)
                        {
                            xmlSerialisedForm.WriteElementString("Item", item);
                        }
                    ...
                    else if (childCtrl is TabControl)
                    {
                        xmlSerialisedForm.WriteElementString("SelectedIndex", ((TabControl)childCtrl).SelectedIndex.ToString());
                    }
                    ...
                    if (ctrlToSet.GetType().ToString() == controlType)
                    {
                        ...
                        switch (controlType)
                        {
                            ...
                            case "System.Windows.Forms.ListBox":
                                ...
                                //Serialise all of the items in the listbox
                                XmlNodeList list = n.SelectNodes("Item");
                                lst.Items.Clear();
                                foreach (XmlNode listitem in list)
                                {
                                    lst.Items.Add(listitem.InnerText);
                                }
                            ...
                            case "System.Windows.Forms.TabControl":
                                ((System.Windows.Forms.TabControl)ctrlToSet).SelectedIndex = Convert.ToInt32(n["SelectedIndex"].InnerText);
                                break;
                        }
